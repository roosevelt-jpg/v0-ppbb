    ...
    fields: [
      {
        ...
        encrypt: false,
        placeholder: 'BA....',
        help: 'Firebase → Project settings → Cloud Messaging → Web Push certificates. Paste the key pair / certificate string that starts with BA…',
      },
      { name: 'serverKey', label: 'Server Key (Legacy)', type: 'password', required: false, encrypt: true, placeholder: 'AAAA...' },
      { name: 'senderId', label: 'Sender ID', type: 'text', required: false, placeholder: '1234567890', help: 'Project number from Firebase Console (optional if Admin SDK already configured)' },
      { name: 'serviceAccountJson', label: 'Service Account JSON', type: 'textarea', required: false, encrypt: true, placeholder: '{"type":"service_account",...}', help: 'Optional — only needed if you are not already using the main Firebase Admin credentials' },
      { name: 'enabled', label: 'Enable Push Notifications', type: 'checkbox', required: false },
    ],
    docs: 'https://firebase.google.com/docs/cloud-messaging',
    help: 'For phone/browser install alerts, the Web Push VAPID key is required. Sender ID and service account are optional when the app already has Firebase Admin configured.',
  },
