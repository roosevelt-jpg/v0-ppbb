...
export default function MarketplaceOfferDetailPage() {
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      const mode = offer?.price != null && offer.price > 0 ? 'purchase' : 'enquire'
      const res = await fetch('/api/marketplace/purchase', {
        ...
        body: JSON.stringify({ offerId, mode, paymentGateway: 'direct' }),
      })
      const json = await res.json()
      if (!json.success) throw new Error(json.error)
      setMessage(mode === 'purchase' ? 'Purchase recorded successfully!' : 'Enquiry sent to the business.')
    } catch (err) {
        ...
        {loading ? (
          ...
          <div className="space-y-8">
            ...
            <div>
              ...
              <p className="text-neutral-700 whitespace-pre-wrap">{offer.description || 'No description provided.'}</p>
            </div>
