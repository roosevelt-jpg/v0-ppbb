...
export async function GET(request: NextRequest) {
  try {
    ...
    const admins = snapshot.docs.map((docSnap) => ({ ...docSnap.data(), id: docSnap.id }))
    return NextResponse.json({ success: true, admins, count: admins.length })
  } catch (error) {
    console.error('[v0] Error fetching admins:', error)
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to fetch admins' }, { status: 500 })
  }
}
