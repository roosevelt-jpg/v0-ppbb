...
export default function AdminChatbotPage() {
  ...
  const getSentimentColor = (sentiment: string) => {
    switch (sentiment) {
      ...
      default:
        return 'bg-neutral-100 dark:bg-muted text-neutral-800 dark:text-foreground'
    }
  }
    ...
    >
      <div className="max-w-7xl mx-auto">
        <div className="mb-6 flex items-start gap-4">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-headline font-bold text-neutral-900 dark:text-foreground">
              Chatbot Management
            </h1>
            <p className="text-neutral-600 dark:text-muted-foreground mt-1 font-body">
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
              <Card className="p-6 border border-neutral-200 dark:border-border">
                <p className="text-sm text-neutral-600 dark:text-muted-foreground font-medium">Total Conversations</p>
                <p className="text-3xl font-bold text-neutral-900 dark:text-foreground mt-2">{conversations.length}</p>
              </Card>

              <Card className="p-6 border border-neutral-200 dark:border-border">
                <p className="text-sm text-neutral-600 dark:text-muted-foreground font-medium">Active</p>
              ...
              <Card className="p-6 border border-neutral-200 dark:border-border">
                <p className="text-sm text-neutral-600 dark:text-muted-foreground font-medium">Resolved</p>
              ...
              <Card className="p-6 border border-neutral-200 dark:border-border">
                <p className="text-sm text-neutral-600 dark:text-muted-foreground font-medium">Escalated</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-1">
                <Card className="border border-neutral-200 dark:border-border flex flex-col h-96">
                  <div className="p-4 border-b border-neutral-200 dark:border-border space-y-3">
                    <div className="relative">
                      <Search className="absolute left-3 top-2.5 w-4 h-4 text-neutral-400 dark:text-muted-foreground" />
                      <input
                        type="text"
                        placeholder="Search conversations..."
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                        className="w-full pl-10 pr-3 py-2 border border-neutral-300 dark:border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 border border-neutral-300 dark:border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                  ...
                  <div className="flex-1 overflow-y-auto divide-y divide-neutral-200">
                    {loading ? (
                      <div className="p-4 text-center text-neutral-500 dark:text-muted-foreground">Loading...</div>
                    ) : filteredConversations.length === 0 ? (
                      <div className="p-4 text-center text-neutral-500 dark:text-muted-foreground text-sm">
                        ...
                        <button
                          ...
                          className={`w-full text-left p-3 transition border-l-2 shadow-none min-h-0 rounded-none font-normal ${
                            selectedConvId === conv.id
                              ? 'border-l-blue-600 !bg-neutral-200 dark:bg-muted !text-neutral-900 dark:text-foreground'
                              : 'border-l-transparent !bg-neutral-100 dark:bg-muted !text-neutral-900 dark:text-foreground hover:!bg-neutral-200'
                          }`}
                        >
                          <h4 className="text-sm font-medium text-neutral-900 dark:text-foreground truncate">
                            {conv.title}
                          </h4>
                          <p className="text-xs text-neutral-500 dark:text-muted-foreground mt-1">{conv.userRole}</p>
              ...
              <div className="lg:col-span-2">
                {selectedConv ? (
                  <Card className="border border-neutral-200 dark:border-border flex flex-col h-96">
                    <div className="p-4 border-b border-neutral-200 dark:border-border">
                      <div className="flex items-start justify-between">
                        <div>
                          <h3 className="font-semibold text-neutral-900 dark:text-foreground">{selectedConv.title}</h3>
                        ...
                        <div className="flex gap-2">
                          <select
                            ...
                            className="px-3 py-1 text-sm border border-neutral-300 dark:border-border rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
                          >
                            <option value="active">Active</option>
                            <option value="resolved">Resolved</option>
                            <option value="escalated">Escalated</option>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3">
                      ...
                      {selectedConv.adminReply && (
                        <div className="flex justify-start mt-4 pt-4 border-t border-neutral-200 dark:border-border">
                    ...
                    {!selectedConv.adminResolved && (
                      <form
                        ...
                        className="p-4 border-t border-neutral-200 dark:border-border"
                      >
                        <div className="flex gap-2">
                          <input
                            type="text"
                            ...
                            className="flex-1 px-3 py-2 border border-neutral-300 dark:border-border rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-neutral-50"
                          />
                          <button
                            type="submit"
                  ...
                  <Card className="p-12 border border-neutral-200 dark:border-border text-center">
                    ...
                    <p className="text-neutral-500 dark:text-muted-foreground">Select a conversation to view details</p>
                  </Card>
                )}
              </div>
            </div>
