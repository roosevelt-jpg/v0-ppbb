      ...
      <AdminPageLayout title="Custom Forms" subtitle="Loading…">
        <div className="space-y-6 animate-pulse">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="h-20 bg-neutral-200 dark:bg-muted rounded-lg" />
            ))}
          </div>
          <div className="h-12 bg-neutral-200 dark:bg-muted rounded w-48" />
          {[1, 2].map((i) => (
            <div key={i} className="h-28 bg-neutral-200 dark:bg-muted rounded-lg" />
          ))}
        </div>
      </AdminPageLayout>
    )
    ...
    <AdminPageLayout title="Custom Forms" subtitle="Create and manage custom forms for different purposes">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-muted-foreground mt-1">Create and manage custom forms for different purposes</p>
        </div>
      ...
      {stats && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          <Card className="p-4">
            <p className="text-sm text-gray-600 dark:text-muted-foreground">Total Forms</p>
            <p className="text-2xl font-bold mt-2">{stats.totalForms}</p>
          </Card>
          <Card className="p-4">
            <p className="text-sm text-gray-600 dark:text-muted-foreground">Active Forms</p>
            <p className="text-2xl font-bold mt-2">{stats.activeForms}</p>
          </Card>
          <Card className="p-4">
            <p className="text-sm text-gray-600 dark:text-muted-foreground">Total Submissions</p>
            <p className="text-2xl font-bold mt-2">{stats.totalSubmissions}</p>
          </Card>
          <Card className="p-4">
            <p className="text-sm text-gray-600 dark:text-muted-foreground">Pending Reviews</p>
            ...
            <Card key={form.id} className="p-4">
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-600 dark:text-muted-foreground mt-1 font-body line-clamp-2">{form.description}</p>
                  <div className="flex flex-wrap gap-2 mt-2">
                    ...
                    <span className="text-xs px-2 py-1 bg-gray-100 dark:bg-muted text-gray-700 dark:text-foreground rounded font-body">
                      {form.submissionCount ?? 0} submissions
                    </span>
                  </div>
                  {form.status === 'active' && form.slug ? (
                    <div className="mt-3 flex flex-col sm:flex-row sm:items-center gap-2">
                      <code className="text-xs bg-neutral-100 dark:bg-muted px-2 py-1 rounded break-all font-body">
