...
export default function AdminOpportunityDetailPage() {
  ...
  return (
    ...
    >
      ...
      {loading ? (
        <p className="text-neutral-500 dark:text-muted-foreground">Loading…</p>
      ) : !normalized ? (
        <p className="text-neutral-500 dark:text-muted-foreground">Opportunity not found.</p>
      ) : (
        <div className="space-y-6">
          <div className="bg-white dark:bg-card border border-[#e4e1da] dark:border-border rounded-xl p-5 sm:p-6">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                <h1 className="text-2xl font-bold text-neutral-900 dark:text-foreground mb-1">{normalized.title}</h1>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  ...
                  className="border border-gray-300 dark:border-border px-4 py-2 rounded-lg text-sm"
                >
                  Close
                </button>
                <a
                  ...
                  className="border border-gray-300 dark:border-border px-4 py-2 rounded-lg text-sm inline-flex items-center"
                >
                  Public page
                </a>
              </div>
            ...
            <details className="mt-4">
              <summary className="cursor-pointer text-sm font-semibold text-neutral-700 dark:text-foreground">
                Job description
              </summary>
              <p className="mt-2 whitespace-pre-wrap text-sm text-neutral-800 dark:text-foreground">
          ...
          <div className="bg-white dark:bg-card border border-[#e4e1da] dark:border-border rounded-xl p-5 sm:p-6">
            ...
            {appsLoading ? (
              <p className="text-sm text-neutral-500 dark:text-muted-foreground">Loading applicants…</p>
            ) : filtered.length === 0 ? (
              ...
              <div className="overflow-x-auto rounded-lg border border-neutral-200 dark:border-border">
                        ...
                        <tr key={app.id} className="border-t border-neutral-100 dark:border-border align-top">
                          <td className="px-4 py-4">
                            <div className="flex gap-3">
                              <div className="w-12 h-12 rounded-md overflow-hidden bg-neutral-200 dark:bg-muted shrink-0 flex items-center justify-center">
                                {photo ? (
                                  ...
                                  <span className="font-semibold text-neutral-700 dark:text-foreground">
                              ...
                              <div className="min-w-0">
                                ...
                                {app.applicantEmail ? (
                                  <p className="text-[11px] text-neutral-400 dark:text-muted-foreground mt-0.5 truncate">
                                    {app.applicantEmail}
                                  </p>
                                ) : null}
                                <p className="text-[11px] text-neutral-400 dark:text-muted-foreground mt-1">
                                  Applied On:{' '}
                                  {applied
                          ...
                          <td className="px-4 py-4">
                            ...
                            <ul className="mt-2 space-y-1 text-xs text-neutral-600 dark:text-muted-foreground">
                              <li className="flex items-center gap-1.5">
                                <Clock className="w-3.5 h-3.5 text-neutral-400 dark:text-muted-foreground" />
                                Volunteer Hours : {hours}
                              </li>
                              <li className="flex items-center gap-1.5">
                                <GraduationCap className="w-3.5 h-3.5 text-neutral-400 dark:text-muted-foreground" />
                                Education : {education}
                              </li>
                              <li className="flex items-center gap-1.5">
                                <Briefcase className="w-3.5 h-3.5 text-neutral-400 dark:text-muted-foreground" />
                            ...
                            {app.resumeUrl ? (
                              ...
                              <span className="mt-3 inline-block text-xs text-neutral-400 dark:text-muted-foreground">
                                No CV uploaded
                              </span>
                            )}
                          </td>
                            ...
                            <div className="flex flex-col items-stretch gap-2 min-w-[140px]">
                              ...
                              {app.status === 'pending' ? (
                                <button
                                  ...
                                  className="inline-flex items-center justify-center h-8 px-3 rounded-md border border-neutral-300 dark:border-border bg-white dark:bg-card text-neutral-700 dark:text-foreground text-[11px] font-semibold disabled:opacity-40"
                                >
                                  Mark viewed
                                </button>
                              ) : null}
