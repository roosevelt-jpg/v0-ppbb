...
import { Save, CheckCircle2, AlertCircle, Upload, Plus, Trash2 } from 'lucide-react'
...
export default function AdminCmsVolunteerPage() {
  ...
  const handleImageUpload = async (file: File) => {
    try {
      setUploading(true)
      const url = await uploadImageToFirebase(file, 'volunteer/cms', { preset: 'content' })
        ...
        <div className="space-y-4 rounded-lg border border-[#e4e1da] bg-white p-4 sm:p-6">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              <label className="inline-flex items-center justify-center gap-2 min-h-[44px] px-4 py-2 border border-neutral-300 rounded cursor-pointer hover:bg-neutral-50 bg-white text-black text-sm font-medium w-full sm:w-auto">
              ...
              {pc.imageURL ? (
                <img
                  ...
                  className="h-20 w-auto max-w-full object-cover rounded border border-neutral-200"
                />
              ) : (
                <span className="text-xs text-neutral-400" style={{ fontFamily: 'Inter, sans-serif' }}>
                  No image yet
                </span>
              )}
            </div>
              ...
              {pc.pillarOptions.map((pillar, index) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="inline-flex items-center justify-center min-h-[44px] min-w-[44px] rounded bg-red-600 text-white hover:bg-red-700"
